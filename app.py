...
# ==========================================
# 2. DEFINITIVE ABSOLUTE PATH DIAGNOSTIC FIX
# ==========================================
# This calculates the absolute folder where app.py lives on your hardware
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_PATH = os.path.join(SCRIPT_DIR, 'gradient_pkl.pkl')

print("\n" + "="*60)
print("🔍 SYSTEM DIAGNOSTIC PATH CHECK:")
...
if os.path.exists(MODEL_PATH):
    try:
        ...
        print("✅ SUCCESS: Found file and GradientBoostingRegressor is loaded!")
    except Exception as e:
        print(f"❌ ERROR: File found but failed to unpack. Details: {e}")
        fallback_mode = True
else:
    print("❌ ERROR: 'gradient_pkl.pkl' was NOT found at that exact target location.")
    print("💡 FIX: Please look at the line above and physically move your .pkl file into that folder.")
    ...
    <div class="w-full max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-8 items-stretch">
        ...
        <div class="md:col-span-5 flex flex-col justify-between p-2">
            <div>
                ...
                <p class="mt-4 text-sm text-slate-400 leading-relaxed max-w-xs">
                    Input hardware metric architecture specifications to evaluate smart retail price indexes using live historical model weights.
                </p>
            ...
            <div class="mt-8 md:mt-0">
                ...
                <div class="glass-panel rounded-2xl p-6 border-l-4 border-l-emerald-500 shadow-xl transition-all duration-300">
                    ...
                    <p class="text-xs text-slate-500 mt-2">
                        {% if fallback %}
                        Simulated calculation based on fallback formulas.
                        {% else %}
        ...
        <div class="md:col-span-7 glass-panel rounded-3xl p-6 sm:p-8 shadow-2xl">
            <form action="/predict" method="POST" class="space-y-5">
                ...
                <div>
                    <label for="Brand" class="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-2">Device Manufacturer Brand</label>
...
def predict():
    try:
        ...
        if not fallback_mode and model is not None:
            # Predict via genuine machine learning model pipeline
            ...
            # Fallback simulator if model file paths mismatch
            predicted_value = (brand_encoded * 55) + (processor_speed * 145) + (ram_size * 32) + (storage_capacity * 0.42) + (screen_size * 12) - (weight * 18)
            if predicted_value < 200: 
                predicted_value = 299.00
